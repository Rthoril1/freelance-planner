...
function getTaskLayouts(dayTasks: Task[], startHour: number) {
  ...
  // 2. Map into clusters based on visual overlap
  ...
  // 3. Within each cluster, sort by priority and determine equal-width columns
...
function SortableTaskCard({ 
  task, companyColor, projectColor, companyName, projectName, onUnschedule, startHour, isAbsolute = false, layout
}: { 
  task: Task, companyColor: string, projectColor: string, companyName: string, projectName: string, 
  onUnschedule?: () => void, startHour: number, isAbsolute?: boolean, layout?: { left: number, width: number }
}) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: task.id });
  let style: React.CSSProperties = { 
    transform: CSS.Transform.toString(transform), transition, 
    opacity: isDragging ? 0.6 : 1, zIndex: isDragging ? 50 : 1 
  };
  ...
  return (
    <div ref={setNodeRef} style={{ ...style, backgroundColor: task.status === 'Completed' ? undefined : projectColor + '0D' }}
...
export default function PlannerPage() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (!over) return;
    const activeTaskId = active.id as string, overId = over.id as string, activeTask = tasks.find(t => t.id === activeTaskId);
    if (!activeTask) return;
    if (days.map(d => format(d, 'yyyy-MM-dd')).includes(overId)) {
      const dayDate = new Date(overId), activeRect = active.rect.current.translated, overRect = over.rect;
      if (activeRect && overRect) {
        const totalHoursFromStart = (activeRect.top - overRect.top) / HOUR_HEIGHT;
        ...
        if (dayDate.getHours() < startHour) dayDate.setHours(startHour, 0, 0, 0);
        if (dayDate.getHours() >= endHour + 1) dayDate.setHours(endHour, 45, 0, 0);
        const scheduledEnd = new Date(dayDate);
        scheduledEnd.setMinutes(dayDate.getMinutes() + activeTask.estimatedDuration * 60);
        updateTask(activeTaskId, { status: 'Scheduled', scheduledStart: dayDate.toISOString(), scheduledEnd: scheduledEnd.toISOString() });
      }
      return;
    }
    if (overId === 'unassigned') { updateTask(activeTaskId, { status: 'Todo', scheduledStart: undefined, scheduledEnd: undefined }); return; }
  };
  const runAutoSchedule = async () => {
    await clearSchedule();
    const scheduled = autoScheduleTasks(useStore.getState().tasks.filter(t => t.status === 'Todo' && !t.parentTaskId), profile, new Date());
      ...
      <div className="flex flex-col space-y-8 animate-in fade-in duration-1000 min-h-[calc(100vh-120px)]">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-2">
          ...
          <div className="flex items-center gap-3">
             <button onClick={clearSchedule} className="px-6 py-3 rounded-2xl border border-slate-200 bg-white text-xs font-bold text-slate-400 hover:text-rose-500 hover:border-rose-100 transition-all active:scale-95">Reset Grid</button>
             <button onClick={runAutoSchedule} className="flex items-center gap-2 px-8 py-3 rounded-2xl bg-primary text-white text-xs font-bold shadow-xl shadow-primary/30 hover:bg-primary/90 transition-all hover:scale-[1.02] active:scale-95"><Wand2 className="h-4 w-4" /> Auto Schedule</button>
          </div>
        </div>
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-4">
                <SortableContext id="unassigned" items={unassignedTasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
                   <DroppableColumn id="unassigned" className="h-full min-h-[400px]">
                      {unassignedTasks.map(task => { const info = getTaskInfo(task.id); return <SortableTaskCard key={task.id} task={task} companyColor={info.companyColor} projectColor={info.projectColor} companyName={info.companyName} projectName={info.projectName} startHour={startHour} />; })}
                ...
                <div className="flex min-w-[900px] min-h-full content-start bg-grid-slate-50">
                   ...
                   <div className="flex-1 flex relative">
                      ...
                      {days.map(day => {
                        ...
                        return (
                          <div key={dayStr} className={cn("flex-1 relative h-full border-r border-slate-50 last:border-r-0 transition-colors duration-500", isOffDay ? "bg-slate-50/30" : "hover:bg-primary/[0.005]")} style={{ minHeight: `${hoursList.length * HOUR_HEIGHT}px` }}>
                                ...
                                <SortableContext id={dayStr} items={dayTasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
                                   {(() => {
                                      ...
                                      return dayTasks.map(t => {
                                         ...
                                         return (
                                           <SortableTaskCard key={t.id} task={t} companyColor={info.companyColor} projectColor={info.projectColor} companyName={info.companyName} projectName={info.projectName} startHour={startHour} isAbsolute layout={layoutMap[t.id]}
